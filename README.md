...
以下のプロンプトで作ったものを動作確認しながら修正しました。

```
小学校4年生向けにPythonのゲームエンジンPyxelの入門書を作成してください。
Re:VIEWで書いてほしいです。このリポジトリはRe:VEIWのテンプレートです。すでに動作確認は終わっているので、文章を書き換えていけばOKです。最初に目次を作成してから順に書いてください。
```
